...
const CustomizedTick = ({ x, y, payload }) => {
  return (
    <text
      ...
      dx={x - 30}
      y={y - 30}
      className="scrollable-bar-chart__custom-tick"
      textAnchor="start"
    >
    ...
    <div className="scrollable-bar-chart__scroll-wrapper">
      <ResponsiveContainer width="100%" height={customHeight}>
        <BarChart
          ...
          margin={{ top: 20, right: 50, left: 20 }}
        >
          <YAxis
            type="category"
            dataKey="name"
          ...
          <Bar dataKey="percentage" className="scrollable-bar-chart__bar">
            {data.map((entry, index) => (
              <Cell
                ...
                key={index}
                className="scrollable-bar-chart__cell"
              />
            ))}
            <LabelList
              dataKey="percentage"
              ...
              className="scrollable-bar-chart__custom-label"
            />
          </Bar>
        </BarChart>
      </ResponsiveContainer>
